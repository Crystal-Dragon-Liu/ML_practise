...
class CrawlThread(threading.Thread):
    ...
    def run(self):
      ...
      #QWidget_SHIT.textBrowser.append('%s------线程启动' % self.name)
      #QWidget_SHIT.QtWidgets.QApplication.processEvents()
      while 1:
        ...
        print("---------准备爬取第%s页----------"%page)
        #QWidget_SHIT.textBrowser.append("---------准备爬取第%s页----------"%page)
        #QWidget_SHIT.QtWidgets.QApplication.processEvents()
        parse_url = self.url+str(page)+'/'
        #print(parse_url)
        content=Build_Request.build_request(parse_url=parse_url)
        self.data_queue.put(content)
      print('%s------线程结束' % self.name)
      #QWidget_SHIT.textBrowser.append('%s------线程结束' % self.name)
      #QWidget_SHIT.QtWidgets.QApplication.processEvents()
...
class ParseThread(threading.Thread):
    ...
    def par(self,data):
        ...
        for i in a_href:
            ...
            #QWidget_SHIT.textBrowser.append("获取"+inf['小区名称' ]+"的信息"+"坐标----lng: "+str(inf['坐标']['lng'])+"  lat: "+str(inf['坐标']['lat']))
            #QWidget_SHIT.QtWidgets.QApplication.processEvents()
            ll.append(inf)
...
def create_crawl_thread(page_queue,data_queue,url):
    ...
    for name in crawl_name:
        ...
        print("创建"+name+"采集线程")
        #QWidget_SHIT.textBrowser.append("创建"+name+"采集线程")
        #QWidget_SHIT.QtWidgets.QApplication.processEvents()
        #print(url)
...
def create_parse_thread(data_queue,l_list,lock,d):
    ...
    for name in parse_name:
        ...
        print("创建"+name+"解析线程")
        #QWidget_SHIT.textBrowser.append("创建"+name+"解析线程")
        #QWidget_SHIT.QtWidgets.QApplication.processEvents()
