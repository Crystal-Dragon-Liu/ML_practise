...
def train(image_in,image_inT):
	...
	with tf.Session() as sess:
		...
		#saver.restore(sess, "data/game.cpk-90000")
		epsl = INIT_EPSL
		while True:
			# get action
			if epsl > FINAL_EPSL:
				epsl -= (INIT_EPSL - FINAL_EPSL) / EXPLORE
			Qa_t = Qa.eval(feed_dict = {image_in : [state_t]})[0]
			...
			if (ch_ep==1 and episode>=0 and episode<=N_EPISODE):
				...
				if (episode>9):
					ave_score = np.mean(score_ALL[episode-10:episode+1])
			...
			state_next = np.append(img, state_t[:, :, 0:3], axis = 2)
            	...
            	# put {s,a,r,s'} experience into the q-bank D
			D.append((state_t, Action_take_t, reward, state_next))
			if len(D) > REPLAY_TOT:
				D.popleft()
			if step > OBSERVE:
				# get m {s,a,r,s'}s
				batch = random.sample(D, BATCH_SIZE)
				state_t_batch = [d[0] for d in batch]
				Action_take_batch = [d[1] for d in batch]
				reward_batch = [d[2] for d in batch]
				state_next_batch = [d[3] for d in batch]

				targ_batch = []
				Q_next_batch = QaT.eval(feed_dict = {image_inT: state_next_batch})

				for i in range(0, len(batch)):
					gamma1=gamma*(reward_batch[i]>-1)
					targ_batch.append(reward_batch[i]*(reward_batch[i]>-1) + gamma1 * np.max(Q_next_batch[i]))
				optimizer.run(feed_dict = {targ : targ_batch, Action_PI : Action_take_batch, image_in : state_t_batch})

				if step%UPDATE_TIME ==0:
					sess.run(copyTargetQN)
				if step % 10000 == 1:
					saver.save(sess, 'data/game.cpk', global_step = step)  # save the model
			...
			if step%1000==0:
				if step>OBSERVE+1:
						L = sess.run(Loss,feed_dict = {targ : targ_batch, Action_PI : Action_take_batch, image_in : state_t_batch})
						print('Loss: ', L)
				print('step: ',step, "epsilon:", epsl, " " ,"action:", action_index, " " ,"reward:", reward)
				if episode>1:
					print('episode: ', episode, 'average score: ',ave_score)
