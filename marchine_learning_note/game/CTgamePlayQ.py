	...
	def rungame(self, act):
		...
		if self.B_loc.top <= dbottom and (self.B_loc.left <=self.dot_loc[0] and self.B_loc.right >=self.dot_loc[0]):
			...
			if self.life==1:
				...
				self.dot_loc[0] = np.random.randint(low=10,high=150)*2
				self.dot_loc[1] = np.random.randint(low=75,high=175)*2
			elif self.life>1:
				reward = 0
				self.life-=1
				self.dot_vec_y = self.dot_vec_y * -1
...
def createQN(x):
	...
	w_fc3 = weight_variable([2816, 256])
	b_fc3 = bias_variable([256])

	w_fc4 = weight_variable([256, OUTPUT_SIZE])
	b_fc4 = bias_variable([OUTPUT_SIZE])
    	## conv1:[19,24,16]  conv2 [8,11,32]
	...
	x2_flat = tf.reshape(x2, [-1, 2816])
	xfc3 = tf.nn.relu(tf.matmul(x2_flat, w_fc3) + b_fc3)

	Qa = tf.matmul(xfc3, w_fc4) + b_fc4
	return Qa,w_conv1,b_conv1,w_conv2,b_conv2,w_fc3,b_fc3,w_fc4,b_fc4



def AIPlayCT(image_in,image_inT):
	# define Qa and QaT
	Qa,w_conv1,b_conv1,w_conv2,b_conv2,w_fc3,b_fc3,w_fc4,b_fc4 = createQN(image_in)
	...
	with tf.Session() as sess:
		...
		saver.restore(sess, "data/game.cpk-3100001")
		epsl = INIT_EPSL
		while True:
			# get action
			if epsl > FINAL_EPSL:
